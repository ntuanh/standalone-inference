...
class Server:
    def __init__(self, config):
        ...
        # slot_queue: publish permits, pre-filled with one token per allowed
        # resident message. An edge must hold a token before it starts transmitting
        # and the cloud returns one as soon as it pulls a message off
        # intermediate_queue, so the number of bodies the broker holds at once is
        # bounded by the token count rather than by the edge count —
        # intermediate_queue's own x-max-length cannot do that, because it is
        # evaluated only after the body has been received in full.
        #
        # The pool is itself length-capped (get_slot_queue_args): a permit pool
        ...
        # The two exclusivity locks, one token each. Only minted when a < 1, i.e.
        # when a batch travels as several chunks and the chunk sequence has to be
        # exclusive at both ends: one edge mid-batch, one cloud assembling. With
        # one message per batch there is nothing to interleave and nothing to
        # merge, so the locks stay unminted and every take is a no-op.
        self._chunked = transport_plan(config)["chunks"] > 1
        for lock in (TRANSFER_LOCK_QUEUE, FETCH_LOCK_QUEUE):
            self.channel.queue_declare(queue=lock, durable=False,
                                       arguments=LOCK_QUEUE_ARGS)
            self.channel.queue_purge(queue=lock)
            if self._chunked:
                self.channel.basic_publish(exchange='', routing_key=lock, body=b'1')
